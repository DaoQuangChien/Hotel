; (function($, window, undefined) {
  ...
  Plugin.prototype = {
              ...
              $.ajax({
                ...
                success: function(result) {
                  if (result.status) {
                    ...
                    that.saveCurrentType(selection.id);
                    that.vars.applyTypeModal.modal('hide');
    ...
    getListType: function(name, id, isCreate) {
      var that = this,
          opts = this.options,
              ...
              if (opts.mode === 'update') {
                result.data.forEach(function(type) {
                  listTypeItem += cardTypeItem.replace(/#{{card-type}}/g, type.name).replace('#{{type-id}}', type.id).replace('#{{active}}', function() {
                    return (id && type.id.toString() === id.toString()) ? 'class="active"' : '';
                  });
                });
              } else {
                result.data.forEach(function(type) {
    ...
    updateTypeInput: function(id, tmpId) {
      var opts = this.options,
          inputType = this.vars.typeSelect.filter(opts.typeSelectId);

      if (tmpId) {
        inputType.data('card-type', {id: id, name: this.vars.listType.find('[data-type-id=' + tmpId + '] a').text(), tmpId: tmpId});
        inputType.val(this.vars.listType.find('[data-type-id=' + tmpId + '] a').text());
      } else {
        inputType.data('card-type', {id: id, name: this.vars.listType.find('[data-type-id=' + id + '] a').text()});
        inputType.val(this.vars.listType.find('[data-type-id=' + id + '] a').text());
      }
    },
    saveCurrentType: function(id) {
      $('[data-type-toggle]').data('current-type', {'id': id, 'name': this.vars.listType.find('[data-type-id=' + id + '] a').text()});
    },
  ...
  $.fn[pluginName] = function(options, param1, param2, param3) {
    var request = [];
    this.each(function() {
      ...
      if (!instance) {
        ...
        request.push(instance[options](param1, param2, param3));
      }
    });
    return request;
  };
